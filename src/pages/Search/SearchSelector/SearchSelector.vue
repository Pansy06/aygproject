  ...
  <div class="clearfix selector">
    <div class="type-wrap logo">
      ...
      <div class="value logos">
        <ul class="logo-list">
          <li v-for="item in trademarkList" :key="item.id" @click="trademarkListHandler(item)">{{ item.tmName }}</li>
    ...
    <div class="type-wrap" v-for="item in attrsList.slice(0, 5)" :key="item.attrId">
      <div class="fl key">{{ item.attrName }}</div>
      <div class="fl value">
        <ul class="type-list">
          <li v-for="(value, index) in item.attrValueList" :key="index" @click="attrInfo(item, value)">
            <a>{{ value }}</a>
          </li>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    trademarkListHandler(item) {
      this.$emit('trademarkInfo', item)
    },
    //平台售卖属性值的点击事件
    attrInfo(item, value) {
      this.$emit('attrInfo', item, value)
    }
  }
}
</script>
